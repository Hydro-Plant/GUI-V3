...
public class NewTimeLapse extends MiniScene {
	...
	private void updateShape() {
		...
		pm.setPosition(Positioning.positioning(positioning, 8)[0] * sizex - sizex * extension_width_factor / 2,
				Positioning.positioning(positioning, 8)[1] * sizey + sizey * extension_height_factor / 2);
		...
		ntls.setPosition(Positioning.positioning(positioning, 0)[0] * sizex + sizex * 0.5 * (1 - ntls_width_factor),
				Positioning.positioning(positioning, 0)[1] * sizey + sizey * 0.5 * (1 - ntls_height_factor));
	}
	...
	public int mouseClick(double mousex, double mousey) {
		if (pm.isPressed(mousex - this.positionx, mousey - this.positiony))
			return 1;
		if (active)
			return ntls.mouseClick(mousex - this.positionx, mousey - this.positiony);
		return -1;
	}

	public void mousePressed(double mousex, double mousey) {
		if (active)
			ntls.mousePressed(mousex - this.positionx, mousey - this.positiony);
	}

	public void mouseReleased(double mousex, double mousey) {
		if (active)
			ntls.mouseReleased(mousex - this.positionx, mousey - this.positiony);
	}

	public void mouseDragged(double mousex, double mousey) {
		if (active)
			ntls.mouseDragged(mousex - this.positionx, mousey - this.positiony);
	}
}
