...
public class TimeLapse extends MiniScene {
	...
	private double from_epoch;
	private double to_epoch;
	...
	public TimeLapse() {
		...
		local_from_date = LocalDate.MIN;
		local_to_date = LocalDate.MIN;

		local_from_time = LocalTime.MIN;
		local_to_time = LocalTime.MIN;
	...
	public static TimeLapse fromData(TimeLapseData tld) {
		TimeLapse tl = new TimeLapse();
		tl.setData(tld);
		return tl;
	}

	public void setData(TimeLapseData tld) {
		this.setDate(LocalDate.parse(tld.date_from), LocalDate.parse(tld.date_to));
		this.setTime(LocalTime.parse(tld.time_from), LocalTime.parse(tld.time_to));

		this.setDuration(Duration.parse(tld.duration));
		this.setFPS(tld.frameRate);
	...
	public boolean isPressed(double mousex, double mousey) {
		return b.isPressed(mousex - positionx, mousey - positiony);
	}
